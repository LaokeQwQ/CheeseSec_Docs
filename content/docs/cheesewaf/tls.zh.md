...
- **数据平面监听**：配置 `server.listen_tls`（默认 `443`）与 `server.listen_http3` 启用 HTTPS 与 HTTP/3 数据平面接入。
- **TLS 版本约束**：支持通过 `min_version` 强制限制最低加密协议版本（如 `1.2` 或 `1.3`）。
- **HSTS 响应头**：开启 `hsts: true` 将在出站响应中自动注入 `Strict-Transport-Security` 请求头。
...
1. **查询 DNS 提供商**：调用 `GET /api/acme/providers` 获取当前支持的 DNS API 认证插件列表（如 Cloudflare、Aliyun 等）。
