...
- **涵盖范围**：IP 黑白名单与地理围栏、Bot 挑战与防刷识别、分片滑动窗口限流，以及 **自定义正则规则引擎（Priority 250）**。
...
- **分析预算耗尽策略（`budget_exhausted_policy`）**：当请求体异常巨大或语法树分析在超时窗口内未能完整结束时，系统将触发安全兜底策略：
  - `auto`：遵循全局 `web_attack` 防护策略等级映射兜底动作。
  - `open`：以业务可用性为先予以放行（记录指标）。
  - `observe`：仅记录观察日志，不执行硬阻断。
  - `closed`：安全优先，触发人机挑战或严格拦截。
...
- **闭环规则沉淀**：人工审核可将确认样本转换为所选的载荷、URI、IP 或指纹规则；站点级自动采纳范围更窄，仅写入该站点的自定义载荷规则。
