...
weight: 60
description: Complete guide to the AST semantic engine, custom regex rules, IP/GeoIP filtering, bot challenges, sharded sliding-window counter rate limits, ACLs, and block pages.
---
...
{{< nav-card title="Rate Limiting" link="/docs/cheesewaf/protection/ratelimit/" desc="High-performance sharded sliding-window counter rate limiting on the Data Plane with queue and drop actions." />}}
{{< nav-card title="Access Control Lists (ACL)" link="/docs/cheesewaf/protection/acl/" desc="Low-overhead linear rule evaluation based on HTTP methods, URI prefixes, and request headers." />}}
