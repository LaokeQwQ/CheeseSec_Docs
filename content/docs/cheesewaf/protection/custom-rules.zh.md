...
自定义规则引擎（Custom Rule Engine）在检测流水线中作为 **Phase 1 预筛选器** 运行（引擎优先级为 **250**）。它先于 AST 语义分析引擎（优先级 290+）执行，主要用于拦截已知的敏感管理路径探测、扫描器指纹特征以及针对特定业务接口的定制化阻断。自定义规则代表管理员显式强意图规则，不受全局 `web_attack: smart` 策略置信度门限抑制。一旦命中带有 `block` 动作的规则，请求将立即短路 403 阻断，避免产生高昂的语法树解析开销。
...
| `pattern` | 字符串 | 正则表达式匹配模式（采用 Go RE2 语法，单条上限 16KB，编译指令上限 4096） |
...
## 确定性意图与免策略过滤（Deterministic Enforcement） {#deterministic}

自定义规则代表管理员的确定性处置意志，而非引擎的概率性弱信号。在数据平面两阶段流水线中：
- **动作直接兑现**：当请求命中自定义规则时，系统**无条件立即执行所配置的处置动作（`block` 阻断、`challenge` 挑战、`log` 仅记录）**。
- **免除门槛过滤**：该动作直接生效，**不受 `protection_policy.web_attack` 策略（如最低严重度门槛、模型置信度要求或聚合风险分阈值）的拦截过滤约束**，确保管理员配置的规则所见即所行。

## 安全校验与约束规范 {#validation-limits}

为防止恶意或低效规则对数据平面造成性能衰竭，CheeseWAF 引擎在加载自定义规则时实施以下严格校验：

- **容量配额限制**：单站点最多配置 **256** 条自定义规则；单次导入文档总大小不得超过 **1MB**；单站点所有规则正则字符串总长度不得超过 **256KB**。导入 YAML 文档必须显式包含 `custom_rules` 根节点，严禁空缺文档。
- **复杂度与抗 ReDoS 保证**：底层依赖 Go 标准库 `regexp`（RE2 保证线性时间复杂度，根除灾难性回溯 ReDoS 隐患）。编译后程序指令数超过 4096 的复杂正则将被拒绝。
- **重复性校验**：系统强制要求单站点内不得存在重复的规则 ID；同时严格禁止存在相同匹配位置与相同正则的规则（即重复的 `location + pattern`）。
- **导出文件名安全净化**：在导出规则包（API `GET /api/rules/export`）时，系统对目标文件名强制执行字符白名单清洗（仅允许 `[A-Za-z0-9._-]`），彻底杜绝 `Content-Disposition` 响应头注入与路径穿越风险。
