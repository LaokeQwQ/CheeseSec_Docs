...
| --- | --- | --- |
| **Web 控制台** | 日常可视化运维、防护策略配置、实时日志检索与攻击态势大屏 | 完成系统初始化后访问配置的 HTTP/HTTPS 管理地址。Docker Compose 在宿主机回环地址以 HTTPS 提供 `https://127.0.0.1:9443/`（远程访问请使用 SSH 隧道）。 |
| **命令行 / TUI** | 纯命令行服务器、跳板机无图形环境下的快速状态排查与配置热修 | 执行 `waf-cli` 或 `cheesewaf cli` 启动终端交互界面 |
...
- **身份字段精确校验**：用户名遵循 CLI 手册中的 3–32 个 ASCII 字符规范。角色必须精确匹配 `apisec.permissions` 中的已配置键；首尾或嵌入 Unicode 空白、控制字符（`Cc`）、格式/不可见字符（`Cf`）以及 `*`、`:` 等权限表达式都会被拒绝。服务端不会自动去空格或改大小写。
